...
window.Widgets['pi-estimation'] = (function () {
  ...
  function mount(container) {
    ...
    container.innerHTML =
      '<div class="pi-widget">' +
        '<div class="pi-widget-row">' +
          '<canvas class="pi-board" width="260" height="260"></canvas>' +
          '<canvas class="pi-chart" width="260" height="260"></canvas>' +
        '</div>' +
        '<div class="pi-stats">' +
        ...
        '<div class="pi-controls">' +
          ...
          '<label class="pi-target-label">points to simulate' +
            '<input type="number" id="pi-target" value="8000" min="100" max="200000" step="100">' +
          '</label>' +
        '</div>' +
      '</div>';
    ...
    var targetInput = container.querySelector('#pi-target');
    ...
    function reset() {
      var requested = parseInt(targetInput.value, 10);
      if (!isFinite(requested) || requested < 10) requested = 8000;
      requested = Math.min(requested, 200000);
      targetInput.value = requested;
      maxPoints = requested;
  ...
  function injectStyles() {
    ...
    style.textContent =
      '.pi-widget-row{display:flex;gap:16px;align-items:flex-start;}' +
      '.pi-board,.pi-chart{flex:0 0 auto;width:260px;height:260px;max-width:48%;border:1px solid var(--rule);background:var(--paper);}' +
      ...
      '.pi-controls{margin-top:12px;display:flex;gap:10px;align-items:center;flex-wrap:wrap;}' +
      ...
      '.pi-target-label{font-family:var(--font-mono);font-size:11px;color:var(--ink-soft);display:flex;flex-direction:column;gap:3px;}' +
      '.pi-target-label input{font-family:var(--font-mono);font-size:12px;padding:5px 8px;border:1px solid var(--rule);background:var(--paper-raise);color:var(--ink);width:100px;}' +
      '.pi-target-label input:focus-visible{outline:2px solid var(--live);outline-offset:1px;}' +
      '@media (max-width:520px){.pi-widget-row{flex-wrap:wrap;}.pi-board,.pi-chart{max-width:100%;}}';
